...
namespace Assignments
{
    internal class MedicalHistory
    {
        ...
        {
            FileStream fs = new FileStream("D:\\Training Works\\Basic Programs\\Assignments\\PatientRecords\\PatientMedicalReport.txt"
                , FileMode.Create, FileAccess.Write);
            fs.Close();
        }
        public  static void MedicalHistoryDetails(MedicalHistory medicalhist)
        {
            FileInfo fi = new FileInfo("D:\\Training Works\\Basic Programs\\Assignments\\PatientRecords\\PatientMedicalReport.txt");
            if (!fi.Exists)
            {
                CreateFile();
            }
